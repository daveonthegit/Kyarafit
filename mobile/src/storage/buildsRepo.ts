...
export async function createBuild(input: CreateBuildInput): Promise<Build> {
  ...
  await database.runAsync(
    `INSERT INTO builds (id, name, character, status, notes, image_url, budget_cents, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, input.name, input.character ?? null, status, input.notes ?? null, input.imageUrl ?? null, input.budgetCents ?? null, now, now]
  );
  const build: Build = {
    id,
    name: input.name,
...
export async function updateBuild(id: string, input: UpdateBuildInput): Promise<Build | null> {
  const database = await initClosetDb();
  const existing = await database.getFirstAsync<{ name: string; character: string | null; status: string; notes: string | null; image_url: string | null; budget_cents: number | null; created_at: string; updated_at: string }>(
  ...
  await database.runAsync(
    ...
    [name, character ?? null, status, notes ?? null, image_url ?? null, budget_cents ?? null, updated_at, id]
  );
  const build: Build = {
    id,
    name,
...
export async function getBuild(id: string): Promise<Build | null> {
  ...
  const row = await database.getFirstAsync<{
    ...
    created_at: string;
    updated_at: string;
  }>(`SELECT id, name, character, status, notes, image_url, budget_cents, created_at, updated_at FROM builds WHERE id = ?`, [id]);
  if (!row) return null;
  return {
    id: row.id,
    name: row.name,
...
export async function linkBuildItems(buildId: string, closetItemIds: string[]): Promise<void> {
  ...
  for (const cid of closetItemIds) {
    await database.runAsync(`INSERT INTO build_item_links (build_id, closet_item_id) VALUES (?, ?)`, [buildId, cid]);
  }
