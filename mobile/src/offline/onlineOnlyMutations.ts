/**
 * Mutations that must never be queued for offline replay — they require a live server round-trip
 * to be meaningful (e.g. minting a one-time upload URL). `useOfflineMutation` always calls these
 ...
 *
 * Keys are Convex function names (`getFunctionName(api.x.y)`), e.g. `"files:generateUploadUrl"`.
 */
