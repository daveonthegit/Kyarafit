    ...
    <Modal visible={visible} animationType="slide" transparent>
      <View className="flex-1 justify-end bg-black/60">
        <View className="max-h-[85%] rounded-t-3xl bg-white px-4 pb-8 pt-4">
          <Text className="text-lg font-semibold text-neutral-900">{t("buildDetail.heroFocalTitle")}</Text>
          <Text className="mt-1 text-sm text-neutral-600">{t("buildDetail.heroFocalHint")}</Text>

          <Pressable
            className="relative mt-4 aspect-[4/5] w-full overflow-hidden rounded-2xl bg-neutral-200"
            onPress={(e) => {
          ...
          >
            ...
            <View
              pointerEvents="none"
              className="absolute h-4 w-4 rounded-full border-2 border-white bg-black/40"
              style={{
                left: `${fx * 100}%`,
                top: `${fy * 100}%`,
                marginLeft: -8,
          ...
          <View className="mt-4 flex-row gap-3">
            <Pressable
              onPress={onClose}
              className="flex-1 items-center rounded-xl border border-neutral-300 py-3 active:opacity-80"
            >
              <Text className="font-semibold text-neutral-800">{t("common.cancel")}</Text>
            </Pressable>
            <Pressable
              onPress={() => {
                onSave(fx, fy);
              ...
              className="flex-1 items-center rounded-xl bg-neutral-900 py-3 active:opacity-90"
            >
