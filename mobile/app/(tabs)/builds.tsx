    ...
    <View style={styles.container}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        {!loading && builds.length === 0 && (
          <Text style={styles.meta}>No builds yet. Create one to link closet items and use them in convention packing.</Text>
        )}
        {builds.map((b) => (
          <Pressable
            key={b.id}
...
const styles = StyleSheet.create({
  ...
  meta: { fontSize: 12, color: colors.meta, paddingHorizontal: layout.screenPaddingX, paddingVertical: 24 },
  buildRow: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingVertical: 20,
  ...
  buildMeta: { fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, color: colors.textTertiary },
  fab: {
    position: 'absolute',
    bottom: 120,
    right: 24,
