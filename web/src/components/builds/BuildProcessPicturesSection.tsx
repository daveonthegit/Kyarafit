...
function ResolveProcessImageUrl({
  doc,
  onResolved,
}: {
  doc: {
...
export function BuildProcessPicturesSection({
  buildId,
  userId,
}: {
  buildId: Id<"builds">;
  ...
  const handleRemove = (id: string) => {
    setRemoveError(null);
    removeMutation({ id: id as Id<"buildProcessPictures">, userId }).catch((e) =>
  ...
  const handleReorder = (orderedIds: string[]) => {
    reorderMutation({
      buildId,
      userId,
      orderedIds: orderedIds as Id<"buildProcessPictures">[],
    }).catch(() => {});
  };
  ...
  return (
    <div>
      {list.map((doc) => (
        <ResolveProcessImageUrl key={doc._id} doc={doc} onResolved={onResolved} />
      ))}
      {removeError && (
