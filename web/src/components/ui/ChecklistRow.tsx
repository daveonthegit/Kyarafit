...
export function ChecklistRow({ label, checked, onToggle, code }: ChecklistRowProps) {
  return (
    ...
    >
      <span
        className={`flex-shrink-0 w-4 h-4 border border-black flex items-center justify-center rounded-sm ${
      ...
      {code && (
        <span className="text-[10px] text-kyar-textTertiary">{code}</span>
      )}
    </button>
  );
}
