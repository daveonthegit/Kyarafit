...
export function MobileNavMenu({ open, onClose }: MobileNavMenuProps) {
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Menu" size="sm">
      <div className="flex flex-col h-full -mx-6 -my-6 bg-[#F4F4F4]">
        <nav className="flex flex-col gap-2 p-4 pt-6 flex-1 overflow-y-auto">
          {NAV_SECTIONS_PRIMARY.map((section) => {
            ...
            return (
              <Link
                ...
                href={section.path}
                onClick={onClose}
                className="group relative flex items-center min-h-[44px] gap-4 px-4 transition-[padding,gap] duration-300 ease-[cubic-bezier(0.33,1,0.68,1)] focus:outline-none focus-visible:ring-2 focus-visible:ring-kyar-accent focus-visible:ring-offset-2 focus-visible:ring-offset-[#F4F4F4] rounded-sm hover:bg-black/5"
              >
                <span
                  className={cn(
          ...
          <div className="my-6 mx-4 border-t border-black/[0.06]" aria-hidden />
