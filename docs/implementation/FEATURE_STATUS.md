...
| Seed data                          | IMPLEMENTED | IMPLEMENTED    | NOT               | —           | —     | NOT              | IMPLEMENTED     | convex/seed.ts (createStarter), web builds page empty state "Load sample data" button                                                                                            |
...
- **Web:** web/src/app/builds/page.tsx — when builds list is empty and user is signed in, shows "Load sample data" button that calls `api.seed.createStarter`; loading and error states.
- **Mobile:** No UI to trigger seed yet (optional follow-up); mutation can be called from Convex dashboard or when mobile adds empty-state CTA.
