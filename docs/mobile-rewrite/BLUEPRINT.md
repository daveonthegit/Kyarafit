...
**Status:** canonical. This is the single source document for the Kyarafit mobile rebuild. It supersedes any prior `PHASE_0_AUDIT.md`, `PHASE_1_FOUNDATION.md`, and `PHASES_2_4_IMPLEMENTATION.md` that previously lived in this folder, and the retired **`NATIVE_BUILT_SCREEN_AUDIT.md`** (its content now lives in §6.1 _Built-screen parity matrix_).

**Last updated:** 2026-04-23 (rolling — see §6 _Rolling implementation status_ and §6.1 _Built-screen parity matrix_; web is now the canonical design-system source via shared token adapters; mobile ThemeProvider now drives NativeWind dark mode; targeted design-system color linting added on migrated mobile surfaces; current built-screen pass includes native font rollout, tab-header typography, collapsible filter panels, `+` FAB creation menus, a summary-first build-detail explorer refresh, drag-scroll lock + finger-following explorer ghosting, the high-volume link-elements surface, planner grouped tasks/events/agenda plus in-row reorder and nest/promote structure controls, planner dependency previews sourced from `workflowDependencies`, shared workflow task editing + template application on planner/build detail, direct create-element/create-material actions inside the build explorer, in-sheet child creation/drill-in actions for explorer selection, element/build destructive-management parity on detail flows, a stronger editorial home hierarchy with actionable next steps, shared nav parity restored for the `Builds` label plus mobile tab icons now deriving from the same shared section semantics as web, native account now includes profile-photo picking/cropping plus live username availability feedback, public build cards now support likes and comment threads natively, public social taps route into a dedicated native public-build viewer modeled on the web public build page instead of the private editor route, the Phase 5 native conventions/packing/itinerary stack replacing the previous browser bridge for Events, native Settings account/subscription/notifications subpages, native Groups / Feed / Discover / Profile stacks replacing the previous overflow browser bridge, and in-app `(public)` read-only routes for public id + share-token viewing (KFM-110).)
...
- Scope: all authenticated product surfaces at full parity. Out of scope: landing pages (`web/src/app/page.tsx`, `web/src/components/landing/`\*), Remotion, `/demo/gallery`, `/dev/seed`, legacy routes `/build-detail` and `/closet` (deprecated in favor of `/b/[buildId]` and `/elements`). **Public + unlisted read-only viewing** is in-scope: ship in-app under [`mobile/app/(public)/`](<../../mobile/app/(public)/>) (see §6.1, KFM-110). **Universal links / OS association** for `https://…/b/…` and `https://…/b/s/…` remains tracked in [FOLLOWUP-public-build-viewer.md](FOLLOWUP-public-build-viewer.md) so installed users open the native viewer instead of the browser when desired.
- Web plan for **public vs unlisted** viewer (parity with private build detail + editorial-informed design): [PUBLIC-BUILD-DETAIL-PLAN.md](../PUBLIC-BUILD-DETAIL-PLAN.md). Mobile backlog for link association and viewer depth: [FOLLOWUP-public-build-viewer.md](FOLLOWUP-public-build-viewer.md).
...
**Built screens:** route-level parity vs web mobile (`< lg`) is summarized in **§6.1** _Built-screen parity matrix_ (summary table, cross-cutting notes, recommended pass order). The phase table under _Rolling implementation status_ stays the authoritative **area / ticket** status.
...
| Phase 8 — Groups / Social         | **In progress**                                    | **Groups** now exist in-tree via [`groups/index`](<../../mobile/app/(app)/groups/index.tsx>), [`groups/new`](<../../mobile/app/(app)/groups/new.tsx>), and [`g/[groupId]`](<../../mobile/app/(app)/g/[groupId].tsx>) with group detail, build assignment, and convention-day linking. **Social** now includes native [`feed`](<../../mobile/app/(app)/feed.tsx>), [`discover`](<../../mobile/app/(app)/discover.tsx>), and [`u/[username]`](<../../mobile/app/(app)/u/[username].tsx>) routes, including follow/unfollow plus public-build cards with likes and comment threads. More now deep-links into these native stacks instead of a browser bridge. **KFM-101 (native):** build owner can **invite by email** + pick **viewer/editor** and **remove** collaborators from Summary on [`DetailBody`](../../mobile/src/screens/build-detail/DetailBody.tsx) via `buildCollaborators.addByEmail` / `remove` (same contract as web). **Still open:** richer public-build social polish beyond cards + viewer, and any future transactional “you were invited” push/email if product adds it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |

### 6.1 Built-screen parity matrix (native ↔ web mobile `< lg`)

Former standalone doc: `NATIVE_BUILT_SCREEN_AUDIT.md` (retired — content merged here 2026-04-23).

**Scope:** Routes already present under [`mobile/app/`](../../mobile/app/) that mirror authenticated or public web product surfaces at the **phone viewport** (`WebAppShell` / `BottomNav` baseline, §1.4a). **Depth before breadth:** tighten these before adding new placeholder flows.

#### Summary

| Native route / group                                            | Web mobile baseline      | Parity  | UX         | Focus next                                                                                                                                                                        |
| --------------------------------------------------------------- | ------------------------ | ------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `(tabs)/index`                                                  | `/home`                  | Partial | Good       | Polish vs evolving web home; one-hand hero; horizontal “following” rails.                                                                                                         |
| `(tabs)/builds`                                                 | `/builds`                | Partial | Good       | Deeper create + downstream build-management parity.                                                                                                                               |
| `(tabs)/elements`                                               | `/elements`              | Partial | Good       | Bulk select, deeper tree accordion, explorer-grade structure tooling (convert/delete on detail already wired).                                                                    |
| `(tabs)/planner`                                                | `/planner`               | Partial | Good       | Denser dependency visualization; drag-grade structure controls.                                                                                                                   |
| `(app)/b/[buildId]`                                             | `/b/[buildId]`           | Partial | Acceptable | Explorer/tool breadth vs web `BuildNodeManagerSection`; owner **collaborator invite/remove** on Summary (KFM-101); do not use for public social taps.                             |
| `(public)/public-builds/[buildId]`, `(public)/b/s/[shareToken]` | Public + unlisted viewer | Partial | Good       | Universal links / OS association ([`FOLLOWUP-public-build-viewer.md`](./FOLLOWUP-public-build-viewer.md)); optional public element drill-in; viewer bundle parity as web evolves. |
| `(tabs)/more`                                                   | `MobileNavMenu`          | Exact   | Good       | Keep nav copy aligned with web only.                                                                                                                                              |
| `(app)/settings/*` hub + stack                                  | `/settings`              | Partial | Good       | RevenueCat paywall + guardrails, push prefs, privacy/storage depth (Phase 7).                                                                                                     |
| `(auth)/*`                                                      | `/auth/*`                | Partial | Good       | Bring sign-up + verify/reset up to the same editorial shell quality as sign-in.                                                                                                   |

#### Cross-cutting

- Convex data shapes are largely correct; **information hierarchy** and **subflow completeness** vs web mobile still vary.
- Remaining work is less “wrong colors” than **missing or shallow parity** (explorer tooling, dependency authoring, billing, optional transactional invite notifications per KFM-089).
- Prefer establishing screen purpose before dense secondary controls (handheld scanability).

#### Per-screen actions

- **Home:** actionable next-step rows; intentional settings/menu access from header flow; hero + primary CTA one-hand reachable.
- **Builds:** collapsible filters + `+` FAB remain baseline; long-press quick actions; poster-style cards.
- **Elements:** FAB + filters; richer tree + bulk + explorer-style edits; lighter filter chrome (chips).
- **Planner:** grouped tree incl. “Elements and other tasks”; quick-add path; agenda/event lighter than desktop calendar chrome.
- **Build detail:** summary-first; extend explorer toward full web action set; spot-check dark mode on new sheets (most surfaces on tokens).
- **More:** native overflow hub (not a link dump).
- **Settings:** grouped rows; keep profile-photo + username availability as baseline; tier/subscription language aligned with web.
- **Social:** card-edge engagement; native comment flows; authenticated build **collaborator invite/remove** on Summary (**KFM-101 shipped**); public taps stay on native viewer + `(public)` deep links per FOLLOWUP.
...
Secondary stacks (conventions, packing, itinerary, groups, `(public)`) already ship; prioritize **depth** on the rows above before new routes.

### Latest implementation log (current cycle)

- **KFM-101 — Native collaborator invite/remove (build detail Summary)**
  - Owner **Team** card always visible on Summary with **Invite** (email + viewer/editor) calling `buildCollaborators.addByEmail` and per-row **Remove** → `buildCollaborators.remove`, aligned with web [`BuildInviteCollaboratorModal`](../../web/src/components/builds/BuildInviteCollaboratorModal.tsx).
- **Phase 5 — Convention detail + list bulk**
  - Native [`conventions/[id]/index`](<../../mobile/app/(app)/conventions/[id]/index.tsx>) **Event management** card: **Archive** (confirm) → `conventions.update` `{ archived: true }`, **Unarchive** when archived, **Delete** (destructive confirm) → `conventions.remove` then `router.replace` to the events list.
  - Native [`conventions/index`](<../../mobile/app/(app)/conventions/index.tsx>) **Select for actions** modal: multi-select over the **current filtered list**, **Archive** / **Unarchive** via `conventions.archiveMany`, bulk **Delete** via `conventions.removeMany`, and an **Undo** bar that recreates deleted rows with `conventions.create` (same contract as web list bulk).
...
- **KFM-051 Element detail screen + status chips + pricing editor**. **Partial, upgraded:** [`elements/[id]/index`](<../../mobile/app/(app)/elements/[id]/index.tsx>) now uses shared-token editorial cards with hero, progress/bucket/status chips, pricing summary, linked-outfit management, workflow grouping, graph sections for parents / children / add-link actions, and **convert/delete** flows (`cosplayNodes.convertType`, `cosplayNodes.remove`). **Still open:** richer **bulk** + **tree** + explorer-grade tooling vs web.
...
- **KFM-062 ✅ Convention detail with day pills** — [`conventions/[id]`](<../../mobile/app/(app)/conventions/[id]/index.tsx>) now drives the native summary/day-plan surface plus **Event management**: archive / unarchive / delete owner actions wired to `conventions.update` and `conventions.remove` (confirm alerts; delete returns to [`/conventions`](<../../mobile/app/(app)/conventions/index.tsx>)).
...
- **KFM-069 ✅ Conventions list bulk actions** — [`conventions/index`](<../../mobile/app/(app)/conventions/index.tsx>) ships multi-select over the filtered list, `conventions.archiveMany` / `conventions.removeMany`, confirm on bulk delete, and **undo** via `conventions.create` per deleted payload (web-parity bulk loop).
...
- **KFM-101 ✅ Invite collaborator (native build detail)**. Owner-only Summary **Team** card: **Invite** opens a bottom sheet (`email` + **viewer/editor** role) calling [`buildCollaborators.addByEmail`](../../convex/buildCollaborators.ts); each row has **Remove** → `buildCollaborators.remove` with confirm. Implementation: [`DetailBody.tsx`](../../mobile/src/screens/build-detail/DetailBody.tsx) (`collaboratorsOwnerPanel` + invite sheet). Matches web [`BuildInviteCollaboratorModal`](../../web/src/components/builds/BuildInviteCollaboratorModal.tsx) behavior (registered user email only; Convex errors surface inline).
