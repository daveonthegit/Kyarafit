...
- Data: `api.groups.`*, `api.groupConventionDays.`*.
...
| Styling        | NativeWind v4 + design-system tokens                                                                                                                                                                                                                                                         | Tailwind config already in [design-system/tailwind.config.js](design-system/tailwind.config.js); web and mobile share the same tokens. Reject Tamagui to avoid a second styling dialect. |
  ...
  ```json
  {
    "expo": {
      "plugins": [[
        ...
        { "buildReactNativeFromSource": true, "useHermesV1": true }
      ]],
      "updates": {
...
mobile/
  app/                                 Expo Router file-based routes
    ...
    (app)/
      ...
      (tabs)/
        _layout.tsx                    tab bar (Home, Outfits, Elements, Planner, More)
        index.tsx                      Home
        builds.tsx
        elements.tsx
        planner.tsx
...
- Route groups:
  ...
  - `(tabs)`: 5 tabs: Home, Outfits, Elements, Planner, More. "More" is a hub screen linking to Events, Groups, Feed, Discover, Settings (to avoid tab overcrowding — web uses a sidebar with 8 items; mobile must reduce).
...
- **Mobile** (`mobile/`): `react-native-purchases` (SDK 9+), wired in root `_layout.tsx` *after* auth is resolved:
  ```ts
  Purchases.configure({ apiKey: Platform.OS === "ios" ? RC_IOS_KEY : RC_ANDROID_KEY, appUserID: session.userId });
  ```
...
- **Dark mode**: driven by tokens in [design-system/rn_tokens.ts](design-system/rn_tokens.ts).
...
- **KFM-004 ✅ NativeWind v4 setup**. Babel plugin, `tailwind.config.js` extending `design-system/tailwind.config.js`, `global.css` with @tailwind directives. Acceptance: a `className` on a `<View>` applies styles.
...
- **KFM-023 ✅ Theme + dark mode** — [`mobile/src/theme/ThemeProvider.tsx`](../../mobile/src/theme/ThemeProvider.tsx); Appearance screen at [`mobile/app/(app)/settings/appearance.tsx`](../../mobile/app/(app)/settings/appearance.tsx).
...
- **KFM-039 Link elements to build** (sheet). **Done:** [`b/link-elements`](../../mobile/app/(app)/b/link-elements.tsx) + `builds.linkNodes`.
...
- **KFM-060 Conventions list + filter archived**.
- **KFM-061 Create convention modal**.
- **KFM-062 Convention detail with day tabs**.
- **KFM-063 Convention edit**.
- **KFM-064 Day plan assign build sheet**.
- **KFM-065 Packing list per day with swipe actions**.
- **KFM-066 Cross-convention packing screen**.
- **KFM-067 Itinerary timeline screen**.
- **KFM-068 Countdown badges on cards**.
...
- **KFM-074 Templates screen + apply-template action**.
- **KFM-075 Today view**.

### Phase 7

- **KFM-080 Settings hub**.
- **KFM-081 Account edit** (profile image crop, username uniqueness).
...
- **KFM-087 Storage usage meter + upgrade CTA** (wired through `design-system/domain/entitlements.ts` `limit(tier, "storage_mb_soft_cap")`).
- **KFM-088 Settings → Notifications, Privacy/visibility, Appearance (theme)**. Scope: Notifications screen with a marketing-push opt-in toggle that writes `userPushPreferences.marketingOptIn` and an informational row for transactional pushes that deep-links to OS notification settings; Privacy/visibility toggles (public profile, share build likes); Appearance with theme picker and the language switcher from KFM-029a. Acceptance: toggling marketing opt-in persists to Convex within 1s and is reflected in the next `broadcasts.deliverDue` run.
...
- **KFM-091 Groups list**.
- **KFM-092 Create group modal**.
- **KFM-093 Group detail** (overview/members/days tabs).
- **KFM-094 Link group to convention days**.
- **KFM-095 Feed screen**.
- **KFM-096 Discover screen**.
- **KFM-097 Profile screen** (`/u/[username]`).
- **KFM-098 Follow/unfollow action**.
- **KFM-099 Like/unlike build**.
- **KFM-100 Comments bottom-sheet thread**.
- **KFM-101 Invite collaborator**.

### Phase 9

- **KFM-110 Public share route** (no auth).
...
4. `rules/offline-first.mdc` — enforces Section 3.13. Draft content:
  ```markdown
   ...
   globs: ["mobile/src/features/{builds,elements,conventions,packing,planner}/**", "mobile/src/offline/**", "convex/**"]
   alwaysApply: true
   ---
