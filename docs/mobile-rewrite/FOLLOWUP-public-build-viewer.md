...
2. **Read-only screens** — Mirror the web public viewer (sections gated by owner toggles); no edit affordances. Reuse strings/layout patterns from authenticated [`DetailBody`](../../mobile/src/screens/build-detail/DetailBody.tsx) only where it stays read-only; do not require sign-in to *view* public/unlisted links.
